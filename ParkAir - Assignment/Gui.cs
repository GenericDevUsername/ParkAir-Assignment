...
namespace ParkAir___Assignment
{
  public class Gui
  {
    ...
    private string GenerateTabs(Gui gui)
    {
      ...
      if (gui.Tabs.Count > 1)
      {
        ...
        foreach (ITab tab in Tabs)
        {
          ...
          tabMiddle += $" {(Tabs[this._tabIndex] == tab ? $"{(Tabs[this._tabIndex].Tabber ? "[" : "\x1b[90m[\x1b[0m")}" : " ")}{tab.TabName}{(Tabs[this._tabIndex] == tab ? $"{(Tabs[this._tabIndex].Tabber ? "]" : "\x1b[90m]\x1b[0m")}" : " ")} │";
          tabBottom += $"{new string('─', tab.TabName.Length + 4)}┴";
          
          if (tab == Tabs[Tabs.Count - 1 < 0 ? 0 : Tabs.Count - 1] && tabBottom.Length - (Tabs[this._tabIndex].Tabber ? 0 : 9) < 92)
          {
          ...
          else if (tab == Tabs[Tabs.Count - 1 < 0 ? 0 : Tabs.Count - 1] && tabBottom.Length - (Tabs[this._tabIndex].Tabber ? 0 : 9) == 92)
          {
    ...
    internal void ScreenUpdate()
    {
      ...
      Console.Title = $"{(this.menuTitle is not null ? $"{this.menuTitle} - " : "")}{Tabs[this._tabIndex].TabName}";
