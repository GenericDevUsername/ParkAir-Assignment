  ...
  public static class SysExport
  {
    private static IEnumerable<SysMessage> LoadLogs()
    {
      ...
      foreach (FileInfo file in files)
      {
        ...
        {
          yield return new SysMessage(log);
        }
      }
    }
    ...
    public static void ExportSingle(string filetype = "TXT")
    {
      ...
      {
        case "TXT":
          foreach (SysMessage log in LoadLogs().OrderBy(s => s.timestamp))
          {
            writer.WriteLine(log.sysString);
          }
          break;
        
        case "CSV":
          writer.WriteLine("priority,version,timestamp,hostname,application,processId,messageId,structuredData,message,severity,messageString");
    ...
    public static void ExportMulti(string filetype = "CSV")
    {
      ...
      Dictionary<string, List<SysMessage>> sortedDictionary = new Dictionary<string, List<SysMessage>>();
      foreach (SysMessage logMessage in LoadLogs().OrderBy(s => s.timestamp))
      {
        string dictionaryKey = logMessage.hostname.ToLower().Replace(" ", "_");
        if (!sortedDictionary.ContainsKey(dictionaryKey)) sortedDictionary.Add(dictionaryKey, new List<SysMessage>());
      ...
      foreach (KeyValuePair<string, List<SysMessage>> sortedItem in sortedDictionary)
      {
        ...
        {
          case "TXT":
            foreach (SysMessage log in sortedItem.Value.OrderBy(s => s.timestamp))
            {
              writer.WriteLine(log.sysString);
            }
            break;
