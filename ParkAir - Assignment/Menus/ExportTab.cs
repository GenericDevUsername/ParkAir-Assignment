...
public class ExportTab : ITab
{
  ...
  private void ChangeFilter(int index, ConsoleKey action)
  {
    ...
    {
      case 2:
        _fileType = Mod(_fileType + (action == ConsoleKey.LeftArrow ? -1 : 1), _fileTypes.Count);
        break;

      case 1:
        _exportType = Mod(_exportType + (action == ConsoleKey.LeftArrow ? -1 : 1), _exportTypes.Count);
        break;
    }
  }
  ...
  public string Screen()
  {
    ...
    screenReturn.Append($"│[{(this._selectint == 1 ? ">" : " ")}] Export Type{new string(' ', 83 - 12 - 18)}[{(this._exportType == 0 ? "X" : " ")}] MULTI  [{(this._exportType == 1 ? "X" : " ")}] SINGLE │\n");
    screenReturn.Append($"│[{(this._selectint == 2 ? ">" : " ")}] File Type{new string(' ', 83 - 10 - 18)}[{(this._fileType == 0 ? "X" : " ")}] TXT    [{(this._fileType == 1 ? "X" : " ")}] CSV    │\n");
    screenReturn.Append($"│{new string(' ', 90)}│\n");
    screenReturn.Append($"│[{(this._selectint == 3 ? ">" : " ")}] Export {(!_exporting ? (this._selectint == 3 ? "(Press Enter)" : "             ") : "(Exporting..)")}{new string(' ', 83 - 17)}│\n");
    screenReturn.Append($"└{new string('─', 90)}┘");
